...
public final class Fact extends Predicate {
    ...
    public Fact(final String value, final double confidenceValue) {
        ...
        double[] conf ={confidenceValue};
        super.setTuple("FACT",tokens,conf);
        if(this.isWaypoint()){
            super.setTuple(this.getPredicateName(),tokens,conf);
        }
    }
    ...
    public boolean isWaypoint(){
        return this.getPredicateName().contains("WAYPOINT");
    }
    public int[] ParseFactToPos(){
        List<Argument> a =this.getArguments();
